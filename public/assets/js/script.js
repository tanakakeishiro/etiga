...
$(function () {
  ...
  // 3. 前/次ボタンの状態を更新
  // 4. URLを更新
  // 5. 最初に表示されたニュースアイテムにフォーカスを移動（キーボード操作の場合のみ）
  //
  // 引数:
  //   - page: 表示するページ番号（数値）
  //   - shouldFocusNewsItem: ニュースアイテムにフォーカスを移動するかどうか（真偽値、デフォルト: true）
  //                         キーボード操作の場合はtrueを指定して、フォーカスをニュースアイテムに移動（ページネーションから外す）
  //                         マウスクリックの場合はfalseを指定して、フォーカスを移動しない
  // 戻り値: なし
  const updatePagination = (page, shouldFocusNewsItem = true) => {
    ...
    // キーボード操作の場合のみ、最初に表示されたニュースアイテムにフォーカスを移動
    // これにより、フォーカスビジブルがページネーションのボタンに当たらなくなる
    // マウスクリックの場合は、フォーカスを移動しない
  ...
  // キーボード操作（Enterキー）の場合は、フォーカスをニュースアイテムに移動してページネーションから外します。
  // マウスクリックの場合は、フォーカスを移動しません。
  //
  // 引数:
  ...
  const handlePaginationClick = (e, getTargetPage) => {
    ...
    if (targetPage && targetPage !== currentPage) {
      ...
      // キーボード操作の場合は、フォーカスをニュースアイテムに移動してページネーションから外すため、shouldFocusNewsItemをtrueに設定
      // マウスクリックの場合は、フォーカスを移動しないため、shouldFocusNewsItemをfalseに設定
      const isKeyboardOperation = e.detail === 0;
      updatePagination(targetPage, isKeyboardOperation);

      // マウスクリックの場合は、クリックした要素からフォーカスを外す
      // これにより、ホバースタイルが残り続けることを防ぎ、次のTab操作で予測可能な位置にフォーカスが移動する
      if (!isKeyboardOperation) {
        // setTimeoutを使用して、ページ更新処理が完了した後にフォーカスを外す
        setTimeout(() => {
          $target[0].blur();
        }, 0);
      }
